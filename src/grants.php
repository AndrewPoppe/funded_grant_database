...
// TODO: Clean this up...
$fundingTypeUserAccess            = json_decode(\REDCap::getData(
	array(
		...
		"filterLogic"             => "[netid]='" . $user_id . "'"
	)
), true)[0]['user_restrict_funding_type'];
...
<head>
	...
	<link rel="stylesheet" type="text/css"
		href="https://cdn.datatables.net/v/dt/jszip-2.5.0/dt-1.10.24/af-2.3.5/b-1.7.0/b-colvis-1.7.0/b-html5-1.7.0/b-print-1.7.0/rg-1.1.2/sb-1.0.1/sp-1.2.2/sl-1.3.3/datatables.min.css" />
	<link rel="stylesheet" type="text/css"
	...
	<script type="text/javascript"
		src="https://cdn.datatables.net/v/dt/jszip-2.5.0/dt-1.10.24/af-2.3.5/b-1.7.0/b-colvis-1.7.0/b-html5-1.7.0/b-print-1.7.0/rg-1.1.2/sb-1.0.1/sp-1.2.2/sl-1.3.3/datatables.min.js"></script>
	<script type="text/javascript"
		src="https://cdn.datatables.net/colreorder/1.5.3/js/dataTables.colReorder.min.js"></script>
	<script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>
	<style>
		table.dataTable tr.dtrg-group.dtrg-level-0 td {
			background-color:
				<?php echo \REDCap::escapeHtml($module->configuration["colors"]["accentColor"]); ?>
			;
			color:
				<?php echo \REDCap::escapeHtml($module->configuration["colors"]["accentTextColor"]); ?>
			;
		}

		div.dtsp-panesContainer tr.selected {
			background-color:
				<?php echo \REDCap::escapeHtml($module->configuration["colors"]["secondaryAccentColor"]); ?>
				!important;
			color:
				<?php echo \REDCap::escapeHtml($module->configuration["colors"]["secondaryTextColor"]); ?>
			;
		}

		div.dtsp-panesContainer tr.selected:hover {
			background-color:
				<?php echo $module->configuration["colors"]["secondaryHoverColor"]; ?>
				!important;
			color:
				<?php echo $module->configuration["colors"]["secondaryHoverTextColor"]; ?>
			;
			cursor: pointer;
		}
...
<body>
	...
	<div id="container" style="padding-left:8%;  padding-right:10%; margin-left:auto; margin-right:auto; ">
		<div id="header">
			...
			<em>You may download grant documents by clicking "download" links below. The use of the grants document
				database is strictly limited to authorized individuals, and you are not permitted to share files or any
				embedded content with other individuals. All file downloads are logged.</em>
			<p style="font-size:small;">If you have comments, questions, or concerns, please send an email to <a
					href="mailto:<?php echo \REDCap::escapeHtml($module->configuration["contact"]["email"]) ?>">
				...
				<tbody>
					...
					foreach ( $grants as $id => $row ) {
						...
						$url             = $module->getUrl("src/download.php?p=$grantsProjectId&id=" .
							...
							$eventId . "&field_name=grants_file", $use_noauth);
								...
								grantSections: {
									contains: {
										...
										init: function (that, fn, preDefined = null) {
											let el = $('<select/>').on('input', function () {
												fn(that, this)
											});
